	...
	if( customers == null || allCustomers == null ) {
		...
		for(var i = 0; i < customers.length; i++) {
			...
			rows.push(<Table.Row key={customer.id}>
				...
				<Table.Cell textAlign='center'>
					<ModalCustomerRegister loadCustomers={() => loadCustomers(pageNum)} customer={customer} pageNum={pageNum} />
				</Table.Cell>
				<Table.Cell textAlign='center'>
		...
		return (
			<Container text style={{ marginTop: '7em' }}>
				...
				<ModalCustomerRegister loadCustomers={() => loadCustomers(pageNum)} customer={null} pageNum={pageNum} />
			</Container>
		);
	}
}
